...
//	private ArrayList<AppInfo> allApps;
	 ...
	 * 未加锁的应用信息
	 */
	private ArrayList<AppInfo> unLockApps;
	...
	private class MyAdapter extends BaseAdapter{
		...
		@Override
		public int getCount() {
			tvDesc.setText("未加锁应用:"+unLockApps.size()+"个");
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if(convertView == null){
				...
				ImageView ivUnlock = (ImageView) view.findViewById(R.id.iv_unlock_list_item);
				// 打包
				vh.ivIcon = ivIcon;
				vh.appName = appName;
				vh.ivUnlock = ivUnlock;
				// 背包
				view.setTag(vh);
			}else{
				view = convertView;
			...
			// 获得数据
			AppInfo appInfo = unLockApps.get(position);
...
//					ta.setFillAfter(true); // 动画执行完以后，保持完成的状态
					...
					ta.setAnimationListener(new AnimationListener() {
						...
						public void onAnimationEnd(Animation animation) {
							...
							String packageName = unLockApps.get(position).packageName; // 获得当前应用的包名
							lockDao.addAppLock(packageName);
