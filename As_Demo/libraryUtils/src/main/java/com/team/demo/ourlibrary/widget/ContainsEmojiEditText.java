package com.team.demo.ourlibrary.widget;
...
import com.team.demo.ourlibrary.R;

import static android.content.ContentValues.TAG;

/**
 * 输入框禁止使用 表情图
 * Created by TranGuility on 16/10/17.
 */
                ...
                if (!resetText) {
                    if (count >= 2) {//表情符号的字符长度最小为2
                        ...
                        if (containsEmoji(input.toString())) {
                            resetText = true;
                            Toast.makeText(mContext, "不支持输入Emoji表情图", Toast.LENGTH_SHORT).show();
